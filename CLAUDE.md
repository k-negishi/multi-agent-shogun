...
version: "3.0"
updated: "2026-02-07"
description: "Claude Code + tmux マルチエージェント並列開発プラットフォーム（戦国軍制階層）"
...
**これは全状況共通の手順です**: 新規開始、圧縮、セッション継続、CLAUDE.mdが見える状態すべて。これらを区別することはできず、区別する必要もありません。**常に同じステップに従ってください。**

1. 自己識別: `tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}'`
2. `mcp__memory__read_graph` — ルール、設定、学習内容を復元 **（将軍/家老/軍師のみ。足軽はスキップ — タスクYAMLで十分）**
3. **指示ファイルを読む**: 将軍→`instructions/shogun.md`、家老→`instructions/karo.md`、足軽→`instructions/ashigaru.md`、軍師→`instructions/gunshi.md`。**絶対にスキップしない** — 会話サマリが存在してもスキップしない。サマリはペルソナ、口調、禁止行動を保存しません。
...
CLAUDE.md（自動ロード）のみを使用した軽量復旧。instructions/*.mdは読まない（コスト削減）。
...
Layer 4: Session context — 揮発性（CLAUDE.md自動ロード、instructions/*.md、/clearで消失）
```
