...
version: "3.0"
updated: "2026-02-07"
description: "Codex CLI + tmux マルチエージェント並列開発プラットフォーム（戦国軍制階層）"
...
**これは全状況共通の手順です**: 新規開始、圧縮、セッション継続、AGENTS.mdが見える状態すべて。これらを区別することはできず、区別する必要もありません。**常に同じステップに従ってください。**

1. 自己識別: `tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}'`
2. `mcp__memory__read_graph` — ルール、設定、学習内容を復元 **（将軍/家老/軍師のみ。足軽はスキップ — タスクYAMLで十分）**
3. **指示ファイルを読む**: 将軍→`instructions/generated/codex-shogun.md`、家老→`instructions/generated/codex-karo.md`、足軽→`instructions/generated/codex-ashigaru.md`、軍師→`instructions/generated/codex-gunshi.md`。**絶対にスキップしない** — 会話サマリが存在してもスキップしない。サマリはペルソナ、口調、禁止行動を保存しません。
...
AGENTS.md（自動ロード）のみを使用した軽量復旧。instructions/*.mdは読まない（コスト削減）。
...
**Write/Edit前に必ずRead。** Codex CLIは未読ファイルへのWrite/Editを拒否します。
...
Layer 4: Session context — 揮発性（AGENTS.md自動ロード、instructions/*.md、/clearで消失）
```
